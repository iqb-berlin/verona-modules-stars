...
import {
  InteractionButtonParams, IconButtonTypeEnum, SelectionOption
...
@Component({
  ...
  template: `
    <div class="interaction-editor">
      ...
      <div class="field">
        ...
        <select [value]="params.buttonType" (change)="updateField('buttonType', $any($event.target).value)">
          @for (opt of buttonTypes; track opt.value) { <option [value]="opt.value">{{ opt.label }}</option> }
        </select>
      </div>
      ...
      <div class="field">
        <label>Layout</label>
        <select [value]="params.layout" (change)="updateField('layout', $any($event.target).value)">
      ...
      <div class="field field-row">
        <label><input type="checkbox" [checked]="params.triggerNavigationOnSelect" (change)="updateField('triggerNavigationOnSelect', $any($event.target).checked)"> Navigation bei Auswahl</label>
      </div>
      <div class="field field-row">
        <label><input type="checkbox" [checked]="params.imageUseFullArea" (change)="updateField('imageUseFullArea', $any($event.target).checked)"> Bild: volle Fläche</label>
      </div>
      <div class="field">
        <label>Text</label>
      ...
      <div class="field">
        ...
        <div class="upload-row">
          @if (params.imageSource) {
            <img [src]="params.imageSource" class="preview-thumb" alt="Btn Img">
        ...
        @for (btn of buttons; track $index) {
          <div class="option-card">
            <div class="option-header">
              ...
              <button class="btn-icon btn-remove-sm" (click)="removeButton($index)">✕</button>
            </div>
            <div class="option-fields">
              <div class="field">
                <label>Text</label>
                <input type="text" [value]="btn.text" (input)="updateButton($index, 'text', $any($event.target).value)">
              </div>
              <div class="field">
                <label>Icon</label>
                <select [value]="btn.icon || ''" (change)="updateButton($index, 'icon', $any($event.target).value || undefined)">
                  <option value="">Kein Icon</option>
                  @for (icon of iconTypes; track icon) { <option [value]="icon">{{ icon }}</option> }
                </select>
              </div>
              <div class="field">
                <label>Bild</label>
                <div class="upload-row">
                  @if (btn.imageSource) {
                    <img [src]="btn.imageSource" class="preview-thumb-sm" alt="Opt">
                    <button class="btn-icon" (click)="updateButton($index, 'imageSource', '')">✕</button>
                  }
              ...
              <div class="field">
                ...
                <div class="upload-row">
                  @if (btn.audioSource) {
                    ...
                    <button class="btn-icon" (click)="updateButton($index, 'audioSource', '')">✕</button>
                  }
                  <label class="btn-upload btn-upload-sm">
                    <span>Audio</span>
                    <input type="file" accept="audio/*" (change)="onOptionAudioSelected($event, $index)" hidden>
                  </label>
                </div>
              </div>
            </div>
  ...
  styles: [`
    ...
    .btn-add { background: rgba(139, 92, 246, 0.2); color: #a78bfa; border: 1px solid rgba(139, 92, 246, 0.3);
    ...
    .option-fields { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }
...
export class InteractionButtonsEditorComponent {
  ...
  iconTypes: IconButtonTypeEnum[] = [
    'CHECK_GREEN', 'CLOSE_RED', 'CLAP_HANDS',
    'SMILEY_1', 'SMILEY_2', 'SMILEY_3', 'SMILEY_4', 'SMILEY_5',
    'ONES', 'TENS'
  ];
