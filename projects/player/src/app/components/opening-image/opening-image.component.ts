...
export class OpeningImageComponent extends InteractionComponentDirective {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (params) {
        ...
        this.localParameters.presentationDurationMS =
  ...
  private scheduleFinishAfterDuration() {
    const duration = Number(
      this.unitService.openingImageParams()?.presentationDurationMS || 0,
    );
  ...
  private finishOpeningFlowAndStartMainAudio() {
    ...
    // Set unconditionally: under the FirstClickLayerEnum an absent value still
    // renders the layer (the guard tests !== 'OFF'), so skipping when unset
    // would let the layer reappear once the opening image is gone.
    ...
    if (main?.audioSource) {
      ...
      this.audioService
        .setAudioSrc({ ...main, audioId: 'mainAudio' })
        .then((ready) => {
          if (ready) {
            // eslint-disable-next-line @typescript-eslint/no-floating-promises
