...
export class InteractionPlaceValueComponent extends InteractionComponentDirective {
  ...
  /** CSS transition duration for click/drag animations in milliseconds */
  private static readonly TRANSFORM_ANIMATION_MS = 1000;
  /** Extra vertical gap between tens icons while stacked in the upper panel (in px) */
  // private static readonly MARGIN_PX = 12;
  ...
  /** Tracks the next available visual slot for tens icons in the upper panel */
  private nextTensSlot = 0;
  /** Tracks the next available visual slot for ones icons in the upper panel */
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const desiredTens = Math.min(
        this.maxNumberOfTens,
        Math.floor(total / 10),
      );
  ...
  private recomputeUpperPanelTransforms(): void {
    ...
    const elMargin = this.marginBetweenElements;
    ...
    const onesMargin = 6;
    const strokeMargin = 10 * 2; // 10px on the left of the stroke + 10px on the right of the stroke
    const onesWidthWithMargin = colW + onesMargin; // 30 + 6 = 36px per column
  ...
  private scheduleLayoutUpdate(
    ...
    const performUpdate = () => {
      ...
      if (ids.length > 0) {
        ...
        }, InteractionPlaceValueComponent.TRANSFORM_ANIMATION_MS);
      }
  ...
  private emitResponses(): void {
    ...
    this.responses.emit([
      ...
      {
        id: this.localParameters?.variableId
          ? `${this.localParameters?.variableId}_TENS`
          : 'PLACE_VALUE_TENS',
        status: 'VALUE_CHANGED',
        value: tensCount,
        relevantForResponsesProgress: true,
